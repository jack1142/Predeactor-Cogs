...
class Writer(MixinMeta):
    ...
    async def ask_for_receivers(self, ctx: commands.Context) -> dict:
        ...
        message = (
            ...
            + "\nNote: I may not be able to find the user if you don't give me his ID."
        )
        ...
        # check if valid
        msg = "I have found the following users, is that right?\n" + "\n".join(
        ...
        if not result:
            if result is not None:  # It is False
                await ctx.send(
                    "Try again, if you want a more precise selection, use ID instead of name, or "
    ...
    async def go_for_letter(self, ctx: commands.Context):
        try:
            ...
            await ctx.send(
                "If so, you'll require the help from CH Postal Company, you can contact them bu "
        ...
        if receivers["missing_contactable"]:
            await ctx.send(
                ...
                + (
                    "\nIf you want to try to contact those,you can try to join CH Postal "
